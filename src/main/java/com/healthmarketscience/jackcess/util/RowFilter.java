...
/**
 * The RowFilter class encapsulates a filter test for a table row. This can be used by the {@link #apply(Iterable)}
...
public abstract class RowFilter implements Predicate<Row> {
    ...
    /**
     * Returns {@code true} if the given table row matches the Filter criteria, {@code false} otherwise.
     *
    ...
    /**
     * Adaptation of this class for {@link Predicate} support. Uses the {@link #matches} method.
     */
    @Override
     ...
     *
     * @param rowPattern Map from column names to the values to be matched. A table row will match the target if
     *            {@code Objects.equals(rowPattern.get(s), row.get(s))} for all column names in the pattern map.
     ...
     * @param valuePattern value to be matched. A table row will match the target if
    ...
    /**
     * Creates a filter which inverts the sense of the given filter (rows which are matched by the given filter will not
     * be matched by the returned filter, and vice versa).
     *
    ...
    private class FilterIterable implements Iterable<Row> {
        ...
        /**
         * Returns an iterator which iterates through the rows of the underlying iterable, returning only rows for which
         * the {@link RowFilter#matches} method returns {@code true}
         */
        @Override
