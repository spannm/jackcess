...
public abstract class CustomLinkResolver implements LinkResolver {
     ...
     * {@link #DEFAULT_IN_MEMORY} and {@link #DEFAULT_TEMP_DIR}.
     */
     ...
     * @param defaultInMemory whether or not the temp db should be entirely in memory by default (while this will be
     *            faster, it should only be used if table data is expected to fit entirely in memory)
     * @param defaultTempDir the default temp dir for a file based temp db ({@code null} for the system defaqult temp
     *            directory)
     */
     ...
     * @param inMemory whether or not the temp db should be entirely in memory (while this will be faster, it should
     *            only be used if table data is expected to fit entirely in memory)
     * @param tempDir the temp dir for a file based temp db ({@code null} for the system default temp directory)
     *
    ...
    /**
     * Called by {@link #resolveLinkedDatabase} to determine whether the linkeeFileName should be treated as a custom
     * file (thus utiliziing a temp db) or a normal access db (loaded via the default behavior). Loads any state
     * necessary for subsequently loading data from linkeeFileName.
     * <p>
     * The returned custom file state object will be maintained with the temp db and passed to {@link #loadCustomTable}
     * whenever a new table needs to be loaded. Also, if this object is {@link Closeable}, it will be closed with the
     * temp db.
     *
     * @param linkerDb the primary database in which the link is defined
     * @param linkeeFileName the name of the linked file
     *
     * @return non-{@code null} if linkeeFileName should be treated as a custom file (using a temp db) or {@code null}
     ...
     *
     * @param tempDb the temp db instance which should be populated with the relevant table info for the given tableName
     * @param customFile custom file state returned from {@link #loadCustomFile}
     * @param tableName the name of the table which is requested from the linked file
     *
