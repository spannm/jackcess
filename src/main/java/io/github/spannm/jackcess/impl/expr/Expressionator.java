...
public class Expressionator {
    ...
    private static final Collection<String> TRUE_STRS  = Collections.unmodifiableList(Arrays.asList("true", "yes", "on"));
    private static final Collection<String> FALSE_STRS = Collections.unmodifiableList(Arrays.asList("false", "no", "off"));
    ...
    private static final Map<OpType, Integer> PRECENDENCE       =
        buildPrecedenceMap(
            ...
            new OpType[] {CompOp.LT, CompOp.GT, CompOp.NE, CompOp.LTE, CompOp.GTE, CompOp.EQ, SpecOp.LIKE, SpecOp.NOT_LIKE, SpecOp.IS_NULL, SpecOp.IS_NOT_NULL},
    ...
    private static final Set<Character>       REGEX_SPEC_CHARS  = Collections.unmodifiableSet(new HashSet<>(Arrays.asList(
        '\\', '.', '%', '=', '+', '$', '^', '|', '(', ')', '{', '}', '&', '[', ']', '*', '?')));
    ...
    /**
     * Converts an ms access like pattern to a java regex, always matching case insensitively.
     */
    ...
    private abstract static class BaseExprWrapper implements Expression {
        ...
        protected Object evalValue(Value.Type resultType, EvalContext ctx) {
            ...
            // FIXME possibly do some type coercion. are there conversions here which don't work elsewhere? (string ->
            // date, string -> number)?
            switch (resultType) {
                case STRING:
    ...
    /**
     * Expression wrapper for an Expr which returns a Boolean from a conditional expression.
     */
