...
public final class IndexData {
    ...
    /** maximum length of the key portion of an index entry. a longer key is truncated, and the bytes truncation discards are replaced with a KEY_DIGEST_LENGTH digest of them */
    ...
    /** polynomial behind KEY_DIGEST_TABLE, the CRC-16/ARC polynomial in its MSB-first form */
    ...
    private byte[] createEntryBytes(Object[] values) throws IOException {
        ...
        return truncateEntryBytes(entryBuffer.toByteArray());
    }

    /**
     * Truncates the given entry bytes if they exceed the maximum key length, replacing the discarded bytes with a digest of them. Access stores long text keys this
     * way, and search keys have to be built the same way to compare against them.
     */
    static byte[] truncateEntryBytes(byte[] entryBytes) {
        ...
        for (int i = KEPT_KEY_LENGTH; i < entryBytes.length - 1; ++i) {
            digest = (digest >>> 8 ^ KEY_DIGEST_TABLE[digest & 0xFF] ^ entryBytes[i] & 0xFF) & 0xFFFF;
        }
    ...
    /**
     * Builds the transition table for the truncated key digest. Byte swapping the digest register turns the transition into the MSB-first CRC-16 step for
     * KEY_DIGEST_POLY, so the table is that step with the register left in on-disk byte order.
     */
    private static int[] createKeyDigestTable() {
        ...
        for (int i = 0; i < table.length; ++i) {
            int reg = i << 8;
            for (int j = 0; j < 8; ++j) {
                reg = (reg << 1 & 0xFFFF) ^ ((reg & 0x8000) != 0 ? KEY_DIGEST_POLY : 0);
            }
